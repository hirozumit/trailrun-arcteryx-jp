  ...
  return (
    <div ref={pageRef} className={styles.page}>
      ...
      <Instruction
        id="prep"
        title={["ギアを", "揃える"]}
        body="都市で行うロードランとは異なり、環境の変化に富む山へと入るトレイルランでは、安全かつ快適に走るための装備が欠かせません。トレイルに適したシューズやウェアのほか、携行品を収納できる軽量なベスト、故障を防ぎ疲労を抑えるスポーツテープ、日中も常に備えておきたいヘッドランプ、すぐに水分補給できるフラスクボトル、救急時にも役立つ手ぬぐい、効率よくエネルギーを摂取できる行動食など。山へと向かう前に、一つひとつ準備と確認を。"
        videos={[{ src: "/videos/yoyogi/instruction-1.mp4", poster: "/images/yoyogi/instruction-1-poster.jpg" }]}
        ctaText="オンラインストア"
        ctaHref="https://arcteryx.jp/collections/trail-run"
      ...
      <section className={styles.about} id="about">
        <div className={styles["about-inner"]}>
          ...
          <div className={styles["about-body"]} data-reveal="fade-up">
            <p>
              アークテリクスによる、トレイルランニングのための準備拠点。代々木公園から徒歩数分の立地で、ロードランの前後にもお立ち寄りいただけます。トレイルランの際に必要となるギアや持ち物、走り方の知識などをチェックしていただけるほか、代々木公園内でトレイルランの練習におすすめのコースもご紹介。アークテリクスの最新のトレイルランニングシューズをレンタルしてお試しいただけます（有料）。また、オープン期間中、トレイルランニング初心者を対象としたイベントも開催します。
            </p>
          </div>
        </div>
          ...
          <div className={styles["pickup-panel"]} data-reveal="clip-left">
            ...
            <div className={styles["pickup-info"]}>
              ...
              <p className={styles["pickup-body"]}>
                トレイルランニング初心者が、山への備えを学ぶことができるTRAIL CLINIC。5/19（火）は夜間の開催で、アークテリクスのトレイルランニングシューズのほか、マイルストーン社のヘッドランプをお試しいただけます。山道は暗くなるのが早いことに加え、緊急時に備えるためにも、ヘッドランプは必携品の一つ。詳しい使用方法などをお伝えしながら、夜間の走り方を学んでいただきます。
              </p>
            </div>
          </div>
        </div>
      ...
      <section className={styles.takao} data-reveal="fade-up">
        <div className={styles["takao-inner"]}>
          ...
          <div className={styles["takao-body"]}>
            <p>
              すべての準備が整ったら、いよいよ山へと足を踏み出すとき。アークテリクスは、都市に暮らす人々にとって最も身近な山の一つである高尾山麓に、トレイルランナーをサポートするための茶屋を期間限定でオープンしました。トレイルランの前後の休憩やエネルギー補給の拠点として、また、山のコンディションやルートの情報交換、最新のギアをお試しいただく場として。高尾の山へと向かう前に、山から下りた後に、どうぞお立ち寄りください。
            </p>
